...
const { EventType, DataLine, Defaults, PatchMode, Namespace } = require('./constants');
 ...
 * @property {string} [viewTransitionSelector] - CSS selector for the element the View Transition is scoped to; only sent when useViewTransition is true (added in 1.0.2)
 * @property {('html'|'svg'|'mathml')} [namespace='html'] - Namespace in which to create new elements (added in 1.0.0-RC.7)
...
class ServerSentEventGenerator {
  ...
  patchElements(elements, options = {}) {
    ...
    if (options.useViewTransition) {
      ...
      // Added in 1.0.2 — the spec only sends this when useViewTransition is true
      if (options.viewTransitionSelector) {
        dataLines.push(`${DataLine.ViewTransitionSelector} ${options.viewTransitionSelector}`);
      }
    }

    // Add namespace if non-default (html is the default)
    if (options.namespace && options.namespace !== Defaults.Namespace) {
      dataLines.push(`${DataLine.Namespace} ${options.namespace}`);
    }
   ...
   * In RC.8+, uses JSON Merge Patch (RFC 7396): setting a signal to null removes it.
  ...
  /**
   * Patch signals only where the keys don't already exist on the client
   * (matches Go SDK's MarshalAndPatchSignalsIfMissing). Equivalent to
   * patchSignals(signals, { onlyIfMissing: true, ...options }).
   * @param {Object|string} signals - Signals object or JSON string
   * @param {PatchSignalsOptions} [options={}] - Additional options
   */
  patchSignalsIfMissing(signals, options = {}) {
    this.patchSignals(signals, { ...options, onlyIfMissing: true });
  }

  /**
   * Remove signals from the client's signal store (RC.8+)
   * Uses JSON Merge Patch (RFC 7396) — setting signals to null deletes them.
   * @param {...string} signalNames - Signal names to remove
   *
   * @example
   * // Remove a single signal
   * sse.removeSignals('tempData');
   *
   * @example
   * // Remove multiple signals
   * sse.removeSignals('tempData', 'loadingState', 'errorMsg');
   */
  removeSignals(...signalNames) {
    const signals = {};
    for (const name of signalNames) {
      signals[name] = null;
    }
    this.patchSignals(signals);
  }

  /**
   * Execute JavaScript in the browser by appending a script element to body
   * In RC.8, the ExecuteScript watcher was removed; scripts are executed by
   * patching a <script> element to body via patchElements.
  ...
  executeScript(script, options = {}) {
    ...
    // In Datastar RC.8, executeScript is done by patching a script element to body
  ...
  /**
   * Remove an element by its id attribute (matches Go SDK's RemoveElementByID)
   * @param {string} id - The id of the element to remove (without leading '#')
   * @param {RemoveElementsOptions} [options={}] - Options
   */
  removeElementByID(id, options = {}) {
    this.removeElement(`#${id}`, options);
  }
