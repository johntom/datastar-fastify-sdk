/**
 * Datastar SDK Constants
 * These values align with the official Datastar specification (1.0.2)
 * Source: https://github.com/starfederation/datastar/blob/v1.0.2/sdk/datastar-sdk-config-v1.json
 * @module constants
 */

'use strict';

/** Current Datastar version */
const DATASTAR_VERSION = '1.0.2';

/** Datastar key used in query parameters and headers */
const DATASTAR_KEY = 'datastar';

/**
 * SSE Event Types
 * Note: In RC.8, RemoveFragments, RemoveSignals, and ExecuteScript watchers
 * were removed. Scripts are executed by patching a <script> element to body.
 * Signal removal is handled via null values in patchSignals (JSON Merge Patch RFC 7396).
 * @enum {string}
 */
const EventType = {
  /** Event for patching HTML elements into the DOM */
  PatchElements: 'datastar-patch-elements',
  /** Event for patching signals (uses JSON Merge Patch RFC 7396 in RC.8+) */
  PatchSignals: 'datastar-patch-signals',
};

/**
 * Element Patch Modes
 * @enum {string}
 */
const PatchMode = {
  /** Morphs the outer HTML of the elements (default and recommended) */
  Outer: 'outer',
  /** Morphs the inner HTML of the elements */
  Inner: 'inner',
  /** Replaces the outer HTML of the elements */
  Replace: 'replace',
  /** Prepends the elements to the target's children */
  Prepend: 'prepend',
  /** Appends the elements to the target's children */
  Append: 'append',
  /** Inserts the elements before the target as siblings */
  Before: 'before',
  /** Inserts the elements after the target as siblings */
  After: 'after',
  /** Removes the target elements from DOM */
  Remove: 'remove',
};

/**
 * Element Namespaces for patchElements (added in 1.0.0-RC.7)
 * @enum {string}
 */
const Namespace = {
  /** HTML namespace (default) */
  HTML: 'html',
  /** SVG namespace */
  SVG: 'svg',
  /** MathML namespace */
  MathML: 'mathml',
};

/**
 * Data Line Prefixes for SSE events
 * @enum {string}
 */
const DataLine = {
  Selector: 'selector',
  Mode: 'mode',
  Elements: 'elements',
  UseViewTransition: 'useViewTransition',
  // Added in 1.0.2 (per sdk/ADR.md and the client patchElements watcher;
  // not yet listed in datastar-sdk-config-v1.json datalineLiterals)
  ViewTransitionSelector: 'viewTransitionSelector',
  Namespace: 'namespace',
  Signals: 'signals',
  OnlyIfMissing: 'onlyIfMissing',
};

/**
 * Default Values
 * @enum {*}
 */
const Defaults = {
  /** Default SSE retry duration in milliseconds */
  SSERetryDuration: 1000,
  /** Default value for useViewTransition */
  UseViewTransitions: false,
  /** Default value for onlyIfMissing when patching signals */
  OnlyIfMissing: false,
  /** Default value for autoRemove when executing scripts */
  AutoRemove: true,
  /** Default element patch mode */
  PatchMode: PatchMode.Outer,
  /** Default element namespace */
  Namespace: Namespace.HTML,
};

/**
 * Request/Response Headers
 * @enum {string}
 */
const Headers = {
  /** Header indicating a Datastar request */
  DatastarRequest: 'datastar-request',
  /** Content type for SSE responses */
  ContentTypeSSE: 'text/event-stream',
  /** Content type for JSON */
  ContentTypeJSON: 'application/json',
  /** Query parameter name for signals in GET requests */
  SignalsQueryParam: 'datastar',
};

module.exports = {
  DATASTAR_VERSION,
  DATASTAR_KEY,
  EventType,
  PatchMode,
  Namespace,
  DataLine,
  Defaults,
  Headers,
};
